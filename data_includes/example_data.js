...
//PennController.GetTable( "SW-datasource-DiscFam.csv" );

PennController.FeedItems( PennController.GetTable( "SW-datasource-DiscFam.csv" ).setLabel("Expt"),//.filter("Expt","experiment"),
    (item) => PennController(
        newTimer("blank", 1000)
            .start()
            .wait()
        ...
        newCanvas("answerbox", 800, 150)
            .settings.add(25,45, newText("labelLeft", "Det &auml;r troligt").settings.size(700, 30).settings.bold() )  
            .settings.add(115,40, getScale("answer").settings.size(200, 0) )
            .settings.add(345,45, newText("labeRight", "inte troligt,").settings.size(700, 30).settings.bold() )
            .settings.add(25,80, newText("claim", item.Likely).settings.size(700, 30) )
